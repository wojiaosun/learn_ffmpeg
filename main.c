...
//    //h264_mp4toannexb
//    //获取名为h264_mp4toannexb的比特流过滤器，这个过滤器的作用是将h.264 MP4格式转换成AnnexB格式
...
//    //声明一个指向AVBSFContext的指针，用于存储比特流的上下文信息
...
//        return -1;
//    }
//    // 将输入媒体格式上下文ifmt_ctx中视频流的编解码参数复制到bsf_ctx的输入参数par_in中
...
//    while(1){
//        ret=av_read_frame(ifmt_ctx,pkt);//不会去释放pkt的buf，如果我们外部不去释放就会出现内存泄露
//        if(ret<0){
//            av_strerror(ret,errors,ERROR_STRING_SIZE);
//            printf("av_read_frame failed:%s\n",errors);
//            break;
//        }
//        //av_read_frame读到数据packet，则外部需要进行buf释放
//        // 检查当前包pkt是否属于视频流
//        if(pkt->stream_index == video_index) {
//            // 处理视频流数据

//            // 将视频包pkt发送给比特流过滤器bsf_ctx进行处理
//            ret = av_bsf_send_packet(bsf_ctx, pkt);
//            // 如果发送失败，打印错误信息，释放pkt占用的资源，并跳过当前循环继续处理下一个包
//            if(ret < 0) {
//                av_strerror(ret, errors, ERROR_STRING_SIZE);
...
//            // 此处不需要释放pkt，因为av_bsf_send_packet已经修改了pkt

//            // 循环接收比特流过滤器处理后输出的包
//            while (1) {
//                // 从比特流过滤器接收一个处理后的包
//                ret = av_bsf_receive_packet(bsf_ctx, pkt);
//                // 如果接收失败（例如，没有更多的包可以接收），则退出循环
//                if(ret != 0) {
//                    break;
//                }
//                // 将过滤器输出的包内容写入到文件中，这里h264_fd是文件描述符
//                size_t size = fwrite(pkt->data, 1, pkt->size, h264_fd);
//                // 如果写入的字节数与pkt->size不同，记录一条日志信息
//                if(size != pkt->size) {
//                    av_log(NULL, AV_LOG_DEBUG, "h264 warning, length of writed data isn't equal pkt->size(%d, %d)\n",
//                           size,
//                           pkt->size);
//                }
//                // 释放由av_bsf_receive_packet返回的包占用的资源
//                av_packet_unref(pkt);
//            }
//        }
