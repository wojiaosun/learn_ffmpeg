...
{
    // 将opaque强制转换为FILE*类型，这是我们用来读取数据的文件指针
    FILE *in_file = (FILE *)opaque;
    // 调用fread函数从in_file中读取数据到buf指定的内存区域
    // 读取的字节数由buf_size指定，返回值read_size表示实际读取的字节数
...
int main(int argc, char **argv)
{
    ...
    // 1. 打开参数文件
    in_file = fopen(in_file_name, "rb");// 以二进制只读模式打开输入文件
    if(!in_file) {
        printf("open file %s failed\n", in_file_name);
        return  -1;
    }
    out_file = fopen(out_file_name, "wb");// 以二进制只写模式打开输出文件
    if(!out_file) {
        printf("open file %s failed\n", out_file_name);
        return  -1;
    }
    ...
    // 创建基于文件的AVIOContext，用于FFmpeg的自定义IO
    AVIOContext *avio_ctx = avio_alloc_context(io_buffer, BUF_SIZE, 0, (void *)in_file,    \
                                              read_packet, NULL, NULL);
    // 分配AVFormatContext，用于存储多媒体文件的格式信息
    AVFormatContext *format_ctx = avformat_alloc_context();
    // 设置AVFormatContext的pb字段为自定义的AVIOContext
    ...
    // 分配解码需要的AVPacket和AVFrame
    AVPacket *packet = av_packet_alloc();
    AVFrame *frame = av_frame_alloc();
 
    while (1) {
        ret = av_read_frame(format_ctx, packet);//读
        if(ret < 0) {
            printf("av_read_frame failed:%s\n", av_err2str(ret));
            break;
        }
        decode(codec_ctx, packet, frame, out_file);//解
    }

    printf("read file finish\n");
    decode(codec_ctx, NULL, frame, out_file);//处理解码器尾部数据，确保解码所有数据
